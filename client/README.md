...
**http://localhost:8080**. The root Compose environment uses the same
**API_URL** name with **http://server:8080**, because Server Actions run inside
the client container and reach Spring Boot through Docker service discovery.
If the root value is omitted, Compose defaults to that internal address.
