...
**API_URL** is intentionally present in both server boundaries. The root
Compose environment uses **http://server:8080** because Next.js runs inside the
client container, while **client/.env.local** uses **http://localhost:8080**
because standalone Next.js runs on the host. Compose defaults to its internal
server address when the root value is omitted.
